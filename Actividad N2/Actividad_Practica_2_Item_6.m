...
Ko=inv(Ro)*Bo'*Po;
eig(Ao'-Ko'*C)
% Fin Cálculo de Controlador Observable------------------------------------
x_hat=[0;0;0;0];
...
omega(ii)=[]; fi(ii)=[]; 
figure(1);hold on;
subplot(2,1,1);plot(t,omega,color);hold on; grid on; title('Velocidad Angular dΦ/dt');xlabel('segundos');hold on;
subplot(2,1,2);plot(t,fi,color);hold on; grid on;title('Ángulo Φ');xlabel('segundos');hold on;
figure(2);hold on;
subplot(2,1,1);plot(t,d,color);hold on;grid on;title('Posición carro δ');xlabel('segundos');hold on;hold on; 
subplot(2,1,2);plot(t,d_p,color);hold on; grid on;title('Velocidad carro dδ/dt');xlabel('segundos');hold on;hold on; 
figure(3);hold on;
plot(t,u,color);hold on; grid on;title('Acción de control');xlabel('segundos');hold on;